...
def classify_url(text):
    ...
    emit(
        "message",
        {
            "msg": "<Classified Last Message from Cain as {} with prob {}".format(
                intent, prob
            )
        },
        room=room,
    ...
    if prob > 0.50:
        ...
        tagged_sent = [
            "({}, {}) ".format(w.text, w.tag_)
            for w in doc
            if w.tag_ in ["NNP", "NN", "NNS"]
        ]
        emit(
            "message",
            {"msg": "<Relevant Tags found are {} >".format(" ".join(tagged_sent))},
            room=room,
        )
        log.append(
            {"msg": "<Relevant Tags found are {} >".format(" ".join(tagged_sent))}
    ...
    if BOT_NAME in msg:
        ...
        context_records = (
            Record.query.filter_by(intent_classification=intent)
        ...
        emit("message", {"msg": "!Bot: {}!".format(resp.get("answer"))}, room=room)
        log.append({"msg": "!Bot: {}!".format(resp.get("answer"))})
        emit(
            "message",
            {
                "msg": "<Bot answered with score {} and with time {}s>".format(
                    resp.get("score"), -start + end
                )
            },
            room=room,
        )
...
def joined(message):
    ...
    emit(
        "status",
        {"msg": session.get("name", "test") + " has entered the room."},
        room=room,
    )
...
def text(message):
    ...
    emit(
        "message",
        {"msg": session.get("name", "test") + ":" + message["msg"]},
        room=room,
    )
    record = Record(log=msg)
    db.session.add(record)
...
def left(message):
    ...
    emit(
        "status",
        {"msg": session.get("name", "test") + " has left the room."},
        room=room,
    )
...
def session_name(message):
    ...
    emit(
        "status",
        {"msg": session.get("name", "test") + " has joined the room."},
        room=room,
    )
